...
// ── Internal protocol prefixes to hide ────────────────────────────────────────
...
function ZapModal({ event, profile, onClose }) {
  ...
  const zap = async (sats) => {
    ...
    try {
      ...
      setErrMsg(e.message || 'Zap failed')
      setStatus('err')
    }
  }

  const handleZap = (sats) => {
    const n = parseInt(sats)
    if (!n || n < 1) { setErrMsg('Enter a valid amount'); return }
    zap(n)
  }
    ...
    >
      <div style={{ width: '100%', maxWidth: 480, background: S.card2, border: `1px solid rgba(201,168,76,0.3)`, borderRadius: '18px 18px 0 0', padding: '20px 20px 44px', animation: 'slideUp .25s ease' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
          ...
          <div>
            <div style={{ fontFamily: 'Cormorant Garamond,serif', fontSize: '1.1rem', fontWeight: 600, color: S.cream }}>
              Zap {profile?.name || profile?.display_name || 'this builder'}
            </div>
            {lnAddress && (
              <div style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: '0.55rem', color: 'rgba(201,168,76,0.4)' }}>
                ⚡ {lnAddress}
              </div>
            )}
          </div>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
          <input
            ...
            onKeyDown={e => e.key === 'Enter' && handleZap(custom)}
          ...
          <button onClick={() => handleZap(custom)} disabled={status === 'fetching' || !custom}
            ...
            {status === 'fetching' ? <Loader size={13} style={{ animation: 'spin 1s linear infinite' }}/> : <ZapIcon size={13}/>}
            Zap
          </button>
        </div>
        ...
        {status === 'done' && (
          <div style={{ padding: '10px 14px', background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.2)', borderRadius: 9, fontFamily: 'Montserrat,sans-serif', fontSize: '0.7rem', color: S.green, display: 'flex', alignItems: 'center', gap: 6 }}>
            <CheckCircle size={13}/> Zap sent — opening wallet…
          </div>
        )}
        {!lnAddress && status === 'idle' && (
          <div style={{ padding: '10px 14px', background: 'rgba(201,168,76,0.05)', border: `1px solid ${S.border}`, borderRadius: 9, fontFamily: 'JetBrains Mono,monospace', fontSize: '0.6rem', color: 'rgba(201,168,76,0.4)' }}>
            // this user has no lightning address set
          </div>
        )}
      </div>
...
function ActionBtn({ onClick, icon, count, activeColor, active, hoverBg }) {
  return (
    <button onClick={onClick} style={{
      ...
      transition: 'all .18s',
      color: active ? activeColor : 'rgba(245,236,215,0.28)',
    }}
    ...
    return parts.map((p, i) => {
      ...
      if (isImageUrl(p)) return (
        <img key={i} src={p} alt="" loading="lazy"
          style={{ display: 'block', maxWidth: '100%', borderRadius: 10, marginTop: 10, border: `1px solid ${S.border}`, cursor: 'pointer' }}
      ...
      >
        ...
        {typeConf.icon && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 10 }}>
            ...
            <span style={{ fontFamily: 'Montserrat,sans-serif', fontSize: '0.58rem', fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', color: typeConf.color }}>
              {typeConf.label}
            </span>
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 12, alignItems: 'center' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontFamily: 'Montserrat,sans-serif', fontSize: '0.82rem', fontWeight: 600, color: S.cream, display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
              {name}
              {profile.nip05 && (
                <span style={{ fontSize: '0.58rem', color: S.gold, display: 'flex', alignItems: 'center', gap: 3 }}>
                  <CheckCircle size={10} /> {profile.nip05}
                </span>
              )}
            </div>
            <div style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: '0.55rem', color: 'rgba(201,168,76,0.4)', marginTop: 2 }}>
          ...
          return (
            <div style={{ marginBottom:14 }}>
              <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:10, marginBottom:8 }}>
                ...
                {reward && <div style={{ textAlign:'right', flexShrink:0 }}>
                  <div style={{ fontFamily:'JetBrains Mono,monospace', fontSize:'0.85rem', fontWeight:700, color:S.gold }}>{parseInt(reward).toLocaleString()}</div>
                  <div style={{ fontFamily:'JetBrains Mono,monospace', fontSize:'0.42rem', color:'rgba(201,168,76,0.4)' }}>sats</div>
                </div>}
              </div>
        ...
        <div style={{ borderTop: `1px solid ${S.border}`, paddingTop: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          {actionMsg && (
            <span style={{
              ...
              color: actionMsg.ok ? S.green : S.red,
...
function ComposeModal({ user, profiles, onClose, onPublished }) {
  ...
  const publish = async () => {
    ...
    try {
      ...
      let signed
      if (window.nostr && !localStorage.getItem('satscode_nsec')) {
    ...
    >
      <div style={{ background: S.card2, border: `1px solid rgba(201,168,76,0.25)`, borderRadius: '16px 16px 0 0', width: '100%', maxWidth: 640, padding: '20px 20px 40px', animation: 'slideUp .25s ease' }}>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 16 }}>
          {TYPE_OPTS.map(t => (
            <button key={t.id} onClick={() => setPostType(t.id)} style={{
              ...
              background: postType === t.id ? `linear-gradient(135deg,${S.gold},${S.goldLight})` : 'rgba(201,168,76,0.05)',
              border: `1px solid ${postType === t.id ? 'transparent' : S.border}`,
              borderRadius: 8, cursor: 'pointer',
              fontFamily: 'Montserrat,sans-serif', fontSize: '0.62rem', fontWeight: postType === t.id ? 700 : 400,
              color: postType === t.id ? S.bg : S.creamFaint,
              transition: 'all .2s',
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <textarea
              ...
              placeholder={
                postType === 'ship'      ? 'What did you ship? Share the link...' :
                postType === 'bounty'    ? 'Describe the bounty and the sat reward...' :
                postType === 'milestone' ? 'Share your milestone...' :
            ...
            <div style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: '0.55rem', color: 'rgba(201,168,76,0.25)', marginTop: 6 }}>
              // posts with #satscode #bitcoin {postType !== 'post' && `#${postType}`}
            </div>
          </div>
        </div>
        ...
        <div style={{ borderTop: `1px solid ${S.border}`, marginTop: 14, paddingTop: 14, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            {status === 'err' && (
              <span style={{ fontSize: '0.62rem', color: S.red, display: 'flex', alignItems: 'center', gap: 4, maxWidth: 160 }}>
                <AlertCircle size={11}/> {errMsg}
              </span>
            )}
            {status === 'ok' && (
              <span style={{ fontSize: '0.62rem', color: S.green, display: 'flex', alignItems: 'center', gap: 4 }}>
                <CheckCircle size={11}/> Published!
              </span>
            )}
            <button onClick={onClose} style={{ background: 'none', border: `1px solid ${S.border}`, color: S.creamFaint, padding: '8px 14px', borderRadius: 8, cursor: 'pointer', fontFamily: 'Montserrat,sans-serif', fontSize: '0.7rem' }}>
              Cancel
            </button>
            <button onClick={publish} disabled={!text.trim() || status === 'busy' || status === 'ok'} style={{
              background: text.trim() ? `linear-gradient(135deg,${S.gold},${S.goldLight})` : 'rgba(201,168,76,0.15)',
              border: 'none', borderRadius: 8, padding: '9px 20px',
              cursor: text.trim() ? 'pointer' : 'not-allowed',
              fontFamily: 'Montserrat,sans-serif', fontSize: '0.75rem', fontWeight: 700,
              letterSpacing: '0.1em', textTransform: 'uppercase',
              color: text.trim() ? S.bg : S.creamFaint,
...
export default function Feed({ user }) {
  ...
  const switchSource = (newSource) => {
    ...
    setLoading(cached.posts.length === 0)
    ...
    const startFeed = (filterOverride) => {
      const sub = pool.subscribe(RELAYS, filterOverride || filter, {
        ...
        oneose() {
          ...
          setLoading(false)
          if (batch.length) {
            cache.posts = [...cache.posts, ...batch.splice(0)].sort((a, b) => b.created_at - a.created_at).slice(0, 100)
            setPosts([...cache.posts])
          }
  ...
  return (
    <>
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '20px 16px 100px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
          <div>
            <h1 style={{ fontFamily: 'Cormorant Garamond,serif', fontSize: '1.8rem', fontWeight: 700, color: S.cream, marginBottom: 4 }}>
              Guild <span style={{ color: S.gold, fontStyle: 'italic' }}>Feed</span>
            </h1>
            <p style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: '0.58rem', color: 'rgba(201,168,76,0.3)', letterSpacing: '0.1em' }}>
              // live · powered by nostr
            </p>
          </div>
          ...
          <div style={{ position: 'relative' }}>
            <button onClick={() => setShowSourceMenu(!showSourceMenu)} style={{
              ...
              fontFamily: 'Montserrat,sans-serif', fontSize: '0.68rem', fontWeight: 500,
                ...
                <div style={{ position: 'absolute', top: '110%', right: 0, zIndex: 100, background: S.card2, border: `1px solid rgba(201,168,76,0.25)`, borderRadius: 10, overflow: 'hidden', minWidth: 180, boxShadow: '0 8px 32px rgba(0,0,0,0.5)', animation: 'fadeUp .15s ease' }}>
                  {SOURCES.map(s => {
                    ...
                    return (
                      <button key={s.id} onClick={() => switchSource(s.id)} style={{
                        ...
                        padding: '11px 14px', background: source === s.id ? 'rgba(201,168,76,0.1)' : 'transparent',
                        border: 'none', cursor: 'pointer',
                        fontFamily: 'Montserrat,sans-serif', fontSize: '0.75rem', fontWeight: source === s.id ? 600 : 400,
                        color: source === s.id ? S.gold : S.creamFaint,
                        borderLeft: source === s.id ? `2px solid ${S.gold}` : '2px solid transparent',
                        transition: 'all .15s',
                      }}>
                  ...
                  <div style={{ padding: '10px 12px', borderTop: `1px solid ${S.border}` }}>
                    ...
                    <div style={{ display: 'flex', gap: 6 }}>
                      ...
                      />
                      <button onClick={applyCustomTag} style={{ background: `linear-gradient(135deg,${S.gold},${S.goldLight})`, border: 'none', borderRadius: 6, padding: '7px 10px', cursor: 'pointer', color: S.bg, fontFamily: 'Montserrat,sans-serif', fontSize: '0.65rem', fontWeight: 700 }}>
                        Go
                      </button>
                    </div>
                  </div>
        ...
        <div style={{ display: 'flex', borderBottom: `1px solid ${S.border}`, marginBottom: 16 }}>
          {TYPE_TABS.map(t => (
            <button key={t.id} onClick={() => { setTypeFilter(t.id); saveFeedPrefs({ ...loadFeedPrefs(), typeFilter: t.id }) }} style={{
              ...
              color: typeFilter === t.id ? S.gold : S.creamFaint,
              padding: '10px 12px', cursor: 'pointer',
              borderBottom: typeFilter === t.id ? `2px solid ${S.gold}` : '2px solid transparent',
        ...
        {newPosts.length > 0 && (
          <button onClick={loadNew} style={{ width: '100%', background: 'rgba(201,168,76,0.06)', border: `1px solid rgba(201,168,76,0.35)`, color: S.gold, padding: 10, borderRadius: 10, fontFamily: 'Montserrat,sans-serif', fontWeight: 600, fontSize: '0.72rem', cursor: 'pointer', marginBottom: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
            <RefreshCw size={13}/>
            {newPosts.length} new post{newPosts.length > 1 ? 's' : ''} — tap to load
          </button>
        )}
        ...
        {!loading && filtered.length === 0 && (
          <div style={{ textAlign: 'center', padding: '60px 0' }}>
            ...
            <div style={{ fontFamily: 'Cormorant Garamond,serif', fontSize: '1.3rem', color: S.creamDim, marginBottom: 8 }}>
              {typeFilter !== 'all' ? `No ${typeFilter}s yet` : 'No posts yet'}
            </div>
            <div style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: '0.62rem', color: 'rgba(201,168,76,0.3)' }}>
              // be the first — tap + to post
            </div>
          </div>
        )}
      ...
      <button onClick={() => setShowCompose(true)} style={{
        ...
        zIndex: 80, transition: 'transform .3s',
      }}
